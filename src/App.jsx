...
function EventFacts() {
  return <div className="wrap meta"><div><strong>{"от 30 секунд"}</strong><span>{"длительность ролика"}</span></div><div><strong>{"10 млн"}</strong><span>{"просмотров — общая цель"}</span></div><div><strong>{"15 декабря"}</strong><span>{"розыгрыш призового фонда"}</span></div></div>;
}
function TeamChoice() {
...
function ParticipationSteps() {
  return <section className="block" style={{
    "paddingTop": "0"
  }}><div className="wrap"><div className="eyebrow">{"02 / Как вступить в игру"}</div><h2>{"От идеи — к просмотрам."}</h2><div className="step-list"><article className="step"><span className="number">{"01"}</span><h3>{"Зарегистрируйся"}</h3><p>{"Открой @Ascauth_bot и выбери сторону: MAGE или HUGE."}</p></article><article className="step"><span className="number">{"02"}</span><h3>{"Сними от 30 секунд"}</h3><p>{"Создай ролик о DONGFENG MAGE или HUGE. Обзор, юмор, впечатления или своя идея — найди подачу, которую захочется досмотреть."}</p></article><article className="step"><span className="number">{"03"}</span><h3>{"Собирай просмотры"}</h3><p>{"Уточни в боте порядок подачи ролика. Учтённые просмотры конкурсных видео увеличивают общий призовой фонд."}</p></article></div></div></section>;
}
function PrizeFund() {
  const [views, setViews] = useState(5);
  return <section className="block fund-section" id="fund"><div className="wrap fund-grid"><div><div className="eyebrow">{"03 / Больше просмотров — больше фонд"}</div><h2>{"Разгоняем"}<br />{"до миллиона."}</h2><p className="formula">{"1 млн просмотров"}<br />{"= +100 000 ₽ в фонд"}</p><p className="lead">{"Наша общая цель — 10 млн просмотров и призовой фонд 1 000 000 ₽. Ролики участников помогают приблизиться к этой цели."}</p><p className="fine">{"Речь об общем призовом фонде конкурса, а не об автоматической выплате каждому автору за просмотры. Максимальный фонд — 1 000 000 ₽."}</p></div><div className="sim"><label htmlFor="views">{"Посмотри, как растёт фонд · пример расчёта"}</label><div className="sim-output"><output id="viewsValue" htmlFor="views">{views}</output>{" млн просмотров"}</div><input id="views" type="range" min="1" max="10" step="1" value={views} onChange={event => setViews(Number(event.target.value))} aria-label="Миллионы просмотров для примера расчёта" /><div className="fund-meter" aria-hidden="true"><span id="fundMeter" style={{
            transform: `scaleX(${views / 10})`
          }} /></div><div className="scale"><span>{"1 млн"}</span><span>{"Цель: 10 млн"}</span></div><div className="sim-result">{"Общий призовой фонд"}<output id="fundValue" htmlFor="views"><strong>{formatFund(views)} ₽</strong></output></div><p className="fine">{"Это пример механики, не счётчик текущих результатов. Каждый полный миллион учтённых просмотров добавляет 100 000 ₽."}</p></div></div></section>;
}
function Registration() {
  return <section className="block" id="registration"><div className="wrap"><div className="registration"><div><div className="eyebrow">{"04 / Твой выход"}</div><h2>{"Камера готова?"}<br />{"Принимай вызов."}</h2><p className="lead">{"Выбери MAGE или HUGE и стань частью противостояния. Первый шаг — регистрация в Telegram-боте АСЦ."}</p><p className="date">{"Розыгрыш — 15 декабря"}</p><p className="fine">{"Порядок учёта просмотров, допуска роликов и распределения призов уточняй у организаторов в боте."}</p></div><a className="telegram" href="https://t.me/Ascauth_bot" target="_blank" rel="noopener"><img src={asset6} alt="Telegram" loading="lazy" /><span className="btn">{"Зарегистрироваться "}<b>{"↗"}</b></span><small>{"Перейти в @Ascauth_bot"}</small></a></div><div className="faq"><details><summary>{"О чём снимать?"}</summary><p>{"О DONGFENG MAGE или HUGE. Длительность конкурсного ролика — от 30 секунд. Команду выбираешь при регистрации."}</p></details><details><summary>{"Каждому заплатят за просмотры?"}</summary><p>{"Просмотры увеличивают общий призовой фонд: 1 млн учтённых просмотров — плюс 100 000 ₽, максимум 1 000 000 ₽. Розыгрыш состоится 15 декабря. Порядок распределения призов уточняется у организаторов."}</p></details><details><summary>{"Где зарегистрироваться и узнать подробности?"}</summary><p>{"В Telegram-боте АСЦ: "}<a href="https://t.me/Ascauth_bot" target="_blank" rel="noopener">{"@Ascauth_bot ↗"}</a>{". Там выбираешь сторону и уточняешь условия участия."}</p></details></div></div></section>;
}
function Footer() {
...
export default function App() {
  ...
  return <div ref={pageRef}><Header /><main><Hero /><EventFacts /><TeamChoice /><ParticipationSteps /><PrizeFund /><Registration /></main><Footer /></div>;
}
