...
function TeamChoice() {
  return <section className="block"><div className="wrap"><div className="eyebrow">{"01 / Выбери свою сторону"}</div><h2>{"Две машины."}<br />{"Твоя история."}</h2><p className="lead">{"MAGE или HUGE — кто станет героем твоего ролика? Выбирай сторону при регистрации в боте и снимай DONGFENG так, как видишь его ты."}</p><div className="teams"><article className="team"><h3><LogoGlint model="MAGE" /><em>{"MAGE"}</em></h3><p>{"Твой взгляд. Твой монтаж. Твоя сторона противостояния."}</p><RegistrationGate className="team-register">{"Хочу в команду MAGE ↗"}</RegistrationGate></article><article className="team huge-team"><h3><LogoGlint model="HUGE" /><em>{"HUGE"}</em></h3><p>{"Сделай HUGE героем истории, которую захочется смотреть."}</p><RegistrationGate className="team-register">{"Хочу в команду HUGE ↗"}</RegistrationGate></article></div><p className="fine">{"Обе кнопки открывают бот регистрации. Окончательный выбор команды — внутри бота."}</p></div></section>;
}
...
export default function App() {
  ...
  return <PrivacyProvider><div ref={pageRef}><Header /><main><Hero /><EventFacts /><TeamChoice /><ParticipationSteps /><PrizeFund /><Engagement /><Registration /><ContestRules /></main><PrivacyLinks /><Footer /></div></PrivacyProvider>;
}
